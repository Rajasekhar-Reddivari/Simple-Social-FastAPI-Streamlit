...
def create_transformed_url(original_url, transformation_params, caption=None):
    ...
    # Be defensive about URL format. Use urllib.parse to avoid IndexError.
    ...
    # If someone provided a URL without scheme starting with ik.imagekit.io, add https
    if not parsed.netloc and parsed.path.startswith("ik.imagekit.io"):
        original_url = "https://" + original_url
        parsed = urlparse(original_url)

    # Remove any existing /tr:.../ segment
    path = parsed.path
    path = re.sub(r"/tr:[^/]+/", "/", path, count=1)
    path = path.lstrip('/')

    if not path or '/' not in path:
        return original_url

    imagekit_id, file_path = path.split('/', 1)
    transformed_path = f"/{imagekit_id}/tr:{transformation_params}/{file_path}"
    new_parsed = parsed._replace(path=transformed_path)
...
def feed_page():
    ...
    if response.status_code == 200:
        ...
        for post in posts:
            ...
            with col1:
                # post['email'] may be a string email or (erroneously) a user dict; normalize to string
                raw_email = post.get('email')
                if isinstance(raw_email, str):
                    display_email = raw_email
                elif isinstance(raw_email, dict):
                    # Try common keys
                    display_email = raw_email.get('email') or raw_email.get('id') or str(raw_email)
                else:
                    display_email = str(raw_email)

                st.markdown(f"**{display_email}** • {post['created_at'][:10]}")
            with col2:
            ...
            if post['file_type'] == 'image':
                # For images: don't overlay the caption on the image. Generate a plain transformed URL
                # (no overlay) and display the caption below the image.
